...
parser.add_argument("-s", "--score-threshold", type=float, default=0)
parser.add_argument("-e", "--fw-epochs", type=int, default=1)
...
for fw_epoch in range(FW_EPOCHS):
    if fw_epoch > 0:
        model = T5ForConditionalGeneration.from_pretrained(model_address).to('cuda')
...
trainer.save_model(model_address)
print('best model dir', model_address)
...
#
# os.system(
#     f'python hf_transformers/tfix_testing.py --load-model {best_model_dir} -bs 8 --model-name t5-large -d repo-based-included -r {repo}')
