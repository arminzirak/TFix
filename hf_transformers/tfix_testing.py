...
tokenizer = T5Tokenizer.from_pretrained(f'{storage_directory}/training/{args.load_model}')
print("Loaded tokenizer from directory {}".format(f'{storage_directory}/training/{args.load_model}'))
model = T5ForConditionalGeneration.from_pretrained(f'{storage_directory}/training/{args.load_model}')
print("Loaded model from directory {}".format(f'{storage_directory}/training/{args.load_model}'))
...
with open(f'{storage_directory}/results.csv', 'a') as f:
    f.write(f'{args.repo if args.repo else "all"},{scores["average"]:.2f},{scores["number_of_warnings"]},{scores["samples_count"]},{dt_string},{model_name},{args.load_model}\n')
