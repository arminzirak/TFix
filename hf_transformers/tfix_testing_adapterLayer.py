...
for i in range(len(encoder_blocks)):
    ...
    encoder_blocks[i].adapter.load_state_dict(torch.load(f'{args.load_model}/adapter-encoder-{i}'))
...
for i in range(len(decoder_blocks)):
    ...
    decoder_blocks[i].adapter.load_state_dict(torch.load(f'{args.load_model}/adapter-decoder-{i}'))
